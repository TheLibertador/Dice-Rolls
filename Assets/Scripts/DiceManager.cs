            ...
            for (int i = 1; i < diceAnimationData[j].Count; i++)
            {
                ...
                for (int k = 1; k < optimizationCount; k++)
                {
                    tempList.Add(new TransformData(
                        Vector3.Lerp(diceAnimationData[j][i - 1].position, nextTransformData.position,
                            (float)k / optimizationCount),
                        Quaternion.Lerp(diceAnimationData[j][i - 1].rotation, nextTransformData.rotation,
                            (float)k / optimizationCount)));
                }
